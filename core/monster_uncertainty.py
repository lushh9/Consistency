...
from core.refinement import REMP_evidential as REMP
...
class Monster(nn.Module):
    ...
    def moe_nig(self, u1, la1, alpha1, beta1, u2, la2, alpha2, beta2):
        # Eq. 9
        la = la1 + la2 # la:[2, 1, 64, 192] la1:[2, 1, 64, 192]
        '''print('la:', la.shape)
        print('la1:', la1.shape)
        print('u1:', u1.shape)
        print('u2:', u2.shape)'''
        u = (la1 * u1 + u2 * la2) / la # u:[2, 1, 64, 192] u1和u2:[2, 1, 64, 192]
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        def get_logits(cost, prob, max_disp):
            #print('cost1:', cost.shape)
            cost = F.upsample(cost, [max_disp//4, image1.size()[2]//4, image1.size()[3]//4], mode='trilinear', align_corners=True)
            cost = cost.squeeze(1) # cost:[2, 48, 64, 192] prob:[2, 48, 64, 192]
            '''print('cost:', cost.shape)
            print('prob:', prob.shape)'''
            pred = torch.sum(cost * prob, 1, keepdim=True) # pred:[2, 1, 64, 192]
            return pred

        cost_volume = self.classifier(geo_encoding_volume)
        logla = self.classifier1(geo_encoding_volume)
        logalpha = self.classifier2(geo_encoding_volume)
        logbeta = self.classifier3(geo_encoding_volume)

        # Init disp from geometry encoding volume
        #prob = F.softmax(self.classifier(geo_encoding_volume).squeeze(1), dim=1)
        prob = F.softmax(cost_volume.squeeze(1), dim=1)
        #print('prob:', prob.shape)
        #print('logla:', logla.shape)
        init_disp = disparity_regression(prob, self.args.max_disp//4)

        logla0 = get_logits(logla, prob, self.args.max_disp)
        logalpha0 = get_logits(logalpha, prob, self.args.max_disp)
        logbeta0 = get_logits(logbeta, prob, self.args.max_disp)
        ...
        for itr in range(iters):
            ...
            if itr == iters - 1:
                refine_value, la1, alpha1, beta1 = self.REMP(disp_mono_4x_up, disp_up, image1, image2)
                
                #prob_refine = F.softmax(refine_value.squeeze(1), dim=1)
                #print('prob_refine:', prob_refine.shape)
                #print('la1:', la1.shape)
                #disp_refine = disparity_regression(prob_refine, self.args.max_disp//4)

                #la1 = get_logits(la1, prob_refine, self.args.max_disp)
                #alpha1 = get_logits(alpha1, prob_refine, self.args.max_disp)
                #beta1 = get_logits(beta1, prob_refine, self.args.max_disp)
                la1, alpha1, beta1 = self.get_uncertainty(la1, alpha1, beta1)
        ...
        (u, la, alpha, beta) = self.combine_uncertainty([[init_disp, la0, alpha0, beta0], [
                disp_up, la1, alpha1, beta1]])

        aleatoric, epistemic = self.compute_uncertainty(la[:, :, :la1.size()[2], :la1.size()[3]], alpha[:, :, :la1.size()[2], :la1.size()[3]], beta[:, :, :la1.size()[2], :la1.size()[3]])

        confidence = {}
        confidence['la'] = la[:, :, :la1.size()[2], :la1.size()[3]]
        confidence['alpha'] = alpha[:, :, :la1.size()[2], :la1.size()[3]]
        confidence['beta'] = beta[:, :, :la1.size()[2], :la1.size()[3]]
        confidence['aleatoric'] = aleatoric
        confidence['epistemic'] = epistemic

        if test_mode:
            return disp_up, confidence

        #init_disp = context_upsample(init_disp*4., spx_pred.float()).unsqueeze(1)
        return init_disp, disp_preds, depth_mono, confidence