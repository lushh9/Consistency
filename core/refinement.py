...
class REMP_evidential(nn.Module):
    ...
    def __init__(self):
        ...
        self.final_conv = nn.Conv2d(32, 1, 3, 1, 1)
        self.final_conv1 = nn.Conv2d(32, 1, 3, 1, 1)
        self.final_conv2 = nn.Conv2d(32, 1, 3, 1, 1)
        self.final_conv3 = nn.Conv2d(32, 1, 3, 1, 1)
    ...
    def forward(self, disp_mono, disp_stereo, left_img, right_img):
        ...
        x_ref = self.final_conv(x)  # [B, 1, H, W]
        la = self.final_conv1(x)  # [B, 1, H, W]
        alpha = self.final_conv2(x)  # [B, 1, H, W]
        beta = self.final_conv3(x)  # [B, 1, H, W]

        disp_stereo = nn.LeakyReLU()(disp_stereo + x_ref)  # [B, 1, H, W]

        return disp_stereo, la, alpha, beta
