...
from core.monster_uncertainty import Monster, autocast
...
from core.utils import dist_utils as comm
...
def sequence_loss(disp_preds, disp_init_pred, disp_gt, valid, loss_gamma=0.9, max_disp=192):
    ...
    def safe_mean(tensor):
        return tensor.mean() if tensor.numel() > 0 else torch.tensor(0.0, device=torch.device)
...
def fetch_optimizer(args, model):
    ...
    rest_params = filter(lambda x:id(x) not in DPT_params and x.requires_grad, model.module.parameters())
...
def main(cfg):
    ...
    model = nn.DataParallel(Monster(cfg))
    ...
    logger = Logger(model, lr_scheduler, cfg.logdir)
    ...
    while should_keep_training:
        ...
        for data in active_train_loader:
            #_, left, right, disp_gt, valid = [x for x in data]

            left = data['img1'].to(torch.device("cuda"))
            right = data['img2'].to(torch.device("cuda"))
            disp_gt = data['disp'].to(torch.device("cuda"))
            valid = data['valid'].to(torch.device("cuda"))
            ...
            print('Epoch:',epoch, 'total_steps:',total_step, 'epe:',metrics['epe'], 'loss:',loss, 'uncertainty_loss:',uncertainty_loss)

            loss = loss + uncertainty_loss

            if loss == 0:
                logger.writer.add_scalar("live_loss", loss, global_batch_num)
            else:
            ...
            if total_step % 20 == 0 and comm.is_main_process():
