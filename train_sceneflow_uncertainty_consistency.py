...
def criterion_uncertainty(u, la, alpha, beta, y, mask):
    ...
    valid_mask = valid.bool() & torch.isfinite(u) & torch.isfinite(la) & torch.isfinite(alpha) & torch.isfinite(
        beta) & torch.isfinite(y)
...
def rank_loss_function_constant(model, sample, path, cfg, accelerator):
    ...
    # _, iter_preds, confidence = model(img1, img2, iters=args.train_iters)
    img1 = sample['img1']
    img2 = sample['img2']
    ...
    mask = ((gt_disp >= 0.5) & (mag < cfg.max_disp))
    ...
    score_batch = confidence['epistemic']
    # pred_disp = disp_pr.detach()

    valid1 = mask.bool() & torch.isfinite(score_batch)
    # valid2 = mask.bool()&torch.isfinite(score_batch)&torch.isfinite(pred_disp)&torch.isfinite(gt_disp)

    score_batch1 = torch.where(valid1, score_batch, torch.tensor(0.0, device=score_batch.device))
    constant_batch = torch.where(valid1, constant_batch, torch.tensor(0.0, device=constant_batch.device))

    '''score_batch2 = torch.where(valid2, score_batch, torch.tensor(0.0, device=score_batch.device))
    pred_disp = torch.where(valid2, pred_disp, torch.tensor(0.0, device=pred_disp.device))
    gt_disp = torch.where(valid2, gt_disp, torch.tensor(0.0, device=gt_disp.device))'''

    constant_batch = constant_batch.flatten()

    constant_batch = (constant_batch - torch.min(constant_batch)) / (
            torch.max(constant_batch) - torch.min(constant_batch) + 1e-7)
    ...
    return CRL_loss_base(constant_batch, score_batch1, 1), disp_pr, confidence, gt_disp
            ...
            for k in range(len(sample["img1_path"])):
                ...
                with lock2:
                    with h5py.File(out_path, 'a') as f2:
                        ...
                        if mask_invalid.any():
                            ...
                            current_disp = torch.where(mask_invalid, torch.from_numpy(f2_temp).to(current_disp.device),
                                                       current_disp)
                ...
                with lock1:
                    with h5py.File(constant_path, 'a') as f1:
                        ...
                        constant_or = torch.where(((torch.from_numpy(f2_temp) - disp_pr[k].cpu()).abs() / (
                                    torch.from_numpy(f2_temp) + 1e-6)) < threshold, 1, 0)
                        if mask_invalid.any():
...
def pearson_correlation_coefficient(confidence, predicted_disp, true_disp):
    ...
    for i in range(confidence.shape[0]):
        ...
        # 计算分母
        denominator = torch.sqrt(torch.sum((error_flat - mean_error) ** 2)) * torch.sqrt(
            torch.sum((confidence_flat - mean_confidence) ** 2))
...
def main(cfg):
    ...
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=1, pin_memory=True, shuffle=False, num_workers=8,
                                             drop_last=False)
    ...
    train_loader, whole_loader, model, optimizer, lr_scheduler, val_loader = accelerator.prepare(train_loader,
                                                                                                 whole_loader, model,
                                                                                                 optimizer,
                                                                                                 lr_scheduler,
                                                                                                 val_loader)
    ...
    while should_keep_training:
        ...
        for data1, data2 in tqdm(zip(active_train_loader, whole_loader), dynamic_ncols=True,
            ...
            loss, metrics = sequence_loss(disp_preds, disp_init_pred, disp_gt, valid, accelerator,
            ...
            constant_loss, whole_disp_preds, whole_confidence, whole_disp_gt = rank_loss_function_constant(model, data2,
                                                                                                           path, cfg,
                                                                                                           accelerator)
            ...
            #
            tqdm.write(
                f"Epoch: {epoch}, total_steps: {total_step}, epe: {metrics['epe']}, loss: {loss}, uncertainty_loss: {uncertainty_loss}, constant_loss: {constant_loss}")
            ...
            if loss.item() != 0.0:
                ...
                accelerator.log({'train/loss': loss, 'train/learning_rate': optimizer.param_groups[0]['lr']},
                                total_step)
            ...
            if total_step % 20 == 0 and accelerator.is_main_process:
                ...
                accelerator.log({"disp_pred": wandb.Image(disp_preds_np, caption="step:{}".format(total_step))},
                                total_step)
                accelerator.log({"disp_gt": wandb.Image(disp_gt_np, caption="step:{}".format(total_step))}, total_step)
                accelerator.log({"depth_mono": wandb.Image(depth_mono_np, caption="step:{}".format(total_step))},
                                total_step)
